...
namespace M.Packages.FileBrowser.Wrapper
{
    ...
    public class FileBrowserWindows : BaseFileBrowser
    {
        ...
        private static int MAX_OPEN_FILE_LENGTH => MAX_FILE_LENGTH * Constants.WINDOWS_MIN_OPEN_NUMBER_OF_FILES;
        ...
        {
            allFilesText = ExtensionFilter._TEXT_ALL_FILES;
        }
        ...
        public override string[] OpenFiles(
            string title, string directory, string defaultName, bool multiselect, params ExtensionFilter[] extensions)
        {
            //resetOpenFiles();
            ...
            try
            {
                ...
                if (multiselect)
                {
                    ...
                    ofn.file = System.Runtime.InteropServices.Marshal.StringToBSTR(fileName +
                        PlatformHelper.CreateString(" ", MAX_OPEN_FILE_LENGTH - fileName.Length));
                    ofn.maxFile = MAX_OPEN_FILE_LENGTH;
                }
                else
                {
                    ofn.file = System.Runtime.InteropServices.Marshal.StringToBSTR(fileName +
                        PlatformHelper.CreateString(" ", MAX_FILE_LENGTH - fileName.Length));
                    ofn.maxFile = MAX_FILE_LENGTH;
                }

                ofn.title = title;
                ofn.flags = (int)FOS.FOS_NOCHANGEDIR | (int)FOS.FOS_FILEMUSTEXIST | (int)FOS.FOS_PATHMUSTEXIST |
                            (multiselect ? (int)FOS.FOS_ALLOWMULTISELECT | OFN_EXPLORER : 0x00000000);
                ...
                if (NativeMethods.GetOpenFileName(ref ofn))
                {
                    ...
                    if (multiselect)
                    {
                        ...
                        if (files.Length > 2)
                        {
                            ...
                            CurrentOpenFiles = selectedFilesList.ToArray();
                            CurrentOpenSingleFile = CurrentOpenFiles[0];

                            return CurrentOpenFiles;
                        }
                    }

                    CurrentOpenSingleFile = Crosstales.Common.Util.FileHelper.ValidateFile(file);
                    // CurrentOpenSingleFile = file;
        ...
        public override string[] OpenFolders(string title, string directory, bool multiselect)
        {
            ...
            if (!PlatformHelper.IsIL2CPP && !FileBrowser.LegacyFolderBrowser)
                Debug.LogWarning("The new folder dialog works only with IL2CPP!");

            //resetOpenFolders();

            string[] paths = !PlatformHelper.IsIL2CPP || FileBrowser.LegacyFolderBrowser
                ? openFolders(directory, false)
                : openFoldersNew(directory, false);

            return paths;
        }

        public override string SaveFile(
            string title, string directory, string defaultName, params ExtensionFilter[] extensions)
        {
            //resetSaveFile();
        ...
        private string[] openFolders(string directory, bool isAsync)
        {
            ...
            CurrentOpenFolders = new[] { path };
            CurrentOpenSingleFolder = path;

            return CurrentOpenFolders;
        }

        private string[] openFoldersNew(string directory, bool isAsync)
        {
            if (!string.IsNullOrEmpty(directory))
            ...
            string title, string directory, string defaultName, params ExtensionFilter[] extensions)
        {
            ...
            try
            {
                ...
                sfn.file = System.Runtime.InteropServices.Marshal.StringToBSTR(fileName +
                                                                               PlatformHelper.CreateString(" ",
                                                                                   MAX_FILE_LENGTH - fileName.Length));
                sfn.maxFile = MAX_FILE_LENGTH;
                sfn.title = title;
                sfn.defExt = defaultExtension;
                sfn.flags = (int)FOS.FOS_NOCHANGEDIR | (int)FOS.FOS_PATHMUSTEXIST |
                            (FileBrowser.AskOverwriteFile ? (int)FOS.FOS_OVERWRITEPROMPT : 0);
        ...
        private static int onBrowseEvent(IntPtr hWnd, int msg, IntPtr lp, IntPtr lpData)
        {
            switch (msg)
            {
                case BFFM_INITIALIZED:
                    NativeMethods.SendMessage(new System.Runtime.InteropServices.HandleRef(null, hWnd),
                        BFFM_SETSELECTIONW, 1, _initialPath);
                    break;
                case BFFM_SELCHANGED:
                {
                    IntPtr pathPtr =
                        System.Runtime.InteropServices.Marshal.AllocHGlobal(260 * System.Runtime.InteropServices.Marshal
                            .SystemDefaultCharSize);

                    if (NativeMethods.SHGetPathFromIDList(lp, pathPtr))
                        NativeMethods.SendMessage(new System.Runtime.InteropServices.HandleRef(null, hWnd),
                            BFFM_SETSTATUSTEXTW, 0, pathPtr);
